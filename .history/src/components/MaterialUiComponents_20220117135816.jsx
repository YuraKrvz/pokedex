...
export function ButtonFavorite({id}){
  ...
  // console.log(id)
  const [state, setState] = React.useState(!!favorites.find(numb => numb !== id));
  const handler = () => {
    // setState(!state)
    dispatch( Toggle(id) )
  }
  React.useEffect(()=>{
    let bool = !!favorites.find(numb => numb === id);
    // console.log(!!favorites.find(numb => numb === id))
    console.log(bool)
    setState(bool)
    // setState(!!favorites.find(numb => numb === id))
  }, [favorites])

  return (
    <IconButton onClick={handler} sx={{color: 'red'}} size="small">
      {state ? <FavoriteIcon fontSize="inherit" /> : <FavoriteBorderIcon fontSize="inherit" />}
    </IconButton>
  )
}
